    ...
    public class AttendanceRecordService : IAttendanceRecordService
    {
        ...
        public async Task<byte[]> ExportMonthlyAttendanceAsync(int year, int month)
        {
            ...
            foreach (var r in records)
            {
                ...
                worksheet.Cell(row, 3).Value = r.Employee?.UserName;
        ...
        public async Task<bool> CheckInAsync(string supervisorId, string employeeId, string shift)
        {
            // Kiểm tra xem supervisor có quản lý employee không
            var groupMembers = await _repo.GetGroupMembersBySupervisorAsync(supervisorId);
            var isInGroup = groupMembers.Any(m => m.UserId == employeeId);
            if (!isInGroup) return false;

            var today = DateOnly.FromDateTime(DateTime.Now);
            var attendance = await _repo.GetAttendanceAsync(employeeId, today);
            ...
            else
            {
                ...
                attendance.Status = GetStatus(now, shift);
        ...
        private string GetStatus(DateTime checkIn, string shift)
        {
            if (shift == "Ca sáng")
            {
                var start = new TimeSpan(5, 0, 0);
                var end = new TimeSpan(5, 15, 0);

                return checkIn.TimeOfDay <= end ? "Đúng giờ" : "Trễ";
            }
            else if (shift == "Ca chiều")
            {
                var start = new TimeSpan(13, 0, 0);
                var end = new TimeSpan(13, 15, 0);
        ...
        public async Task<List<User>> GetEmployeesAsync(string supervisorId)
        {
            return await _repo.GetEmployeesBySupervisorAsync(supervisorId);
        }

        public async Task<bool> SaveAttendancesAsync(string supervisorId, List<string> presentEmployeeIds, string shift)
        {
            var employees = await _repo.GetEmployeesBySupervisorAsync(supervisorId);
            var today = DateOnly.FromDateTime(DateTime.Now);

            foreach (var emp in employees)
            {
                var record = await _repo.GetAttendanceAsync(emp.UserId!, today);
                var now = DateTime.Now;

                if (presentEmployeeIds.Contains(emp.UserId!)) // supervisor tick "Có mặt"
                {
                    if (record == null)
                    {
                    ...
                    {
                        record.CheckInTime = now;
                        record.Status = GetStatus(now, shift);
                        record.UpdatedAt = DateTime.Now;
                        await _repo.UpdateAttendanceAsync(record);
                    }
                }
                else // supervisor không tick → Vắng
                {
                    if (record == null)
                    {
                        record = new AttendanceRecord
